...
ACCEPTANCE_CRITERION|AC-09|Клиент может отменить заказ в статусе NEW и сток восстанавливается|npm run build 2>&1 | tail -1|Build completed successfully
...
ACCEPTANCE_CRITERION|AC-22|При сбое Resend в sendEmail вызывается sendTelegram с сообщением об ошибке|grep -n "sendTelegram" /Users/nurdauletakhmatov/Desktop/alashed-workspace/alash-electronics/frontend/src/lib/email.ts | grep -c "Email failed"|1
