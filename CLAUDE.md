...
Alash Electronics — Next.js 14 e-commerce platform for an electronics store in Kazakhstan (alash-electronics.kz). Russian-language UI. Fully self-hosted (migrated from InSales).
...
**Google Indexing API** (опционально, скрипты в `frontend/scripts/`):

```bash
npm run indexing:google:daily          # ежедневная порция (до 200 URL), state на диске
npm run indexing:google:daily:dry      # dry-run
npm run indexing:google:stats          # сколько осталось / режим «круга»
npm run indexing:google                # разовая массовая отправка (google-index-urls.js)
```

Переменные: см. `frontend/.env.example` (`GOOGLE_INDEXING_KEY_PATH`, `GOOGLE_INDEXING_STATE_PATH`, `SITE_URL`). Ключ сервисного аккаунта **не коммитить**.

## Architecture

**Stack**: Next.js 14 (App Router), React 18, TypeScript, PostgreSQL + Prisma, NextAuth.js (JWT + Credentials), Tailwind CSS, AWS S3 (`alashed-media` bucket, `eu-north-1`), web-push for PWA notifications.

### Key Directories (`frontend/src/`)

- `app/` — Pages and API routes (App Router). Public pages are server components.
- `app/admin/` — Admin dashboard (dark sidebar layout, requires ADMIN role via `lib/admin.ts`)
- `app/api/admin/` — Admin API routes (all use `checkAdmin()` from inline session check)
- `lib/data.ts` — Centralized DB queries: search, pagination, category traversal (~15 functions)
...
- `lib/push.ts` — Web-push notification utility (`notifyAdmins()`)
...
- `components/` — Shared UI (Header, Footer, ProductCard, SubcategoryFilter, RichTextEditor, SafeHtml)
- `prisma/schema.prisma` — 15 models, self-referential Category tree
...
No server-side cart. localStorage keys: `alash_cart`, `alash_favorites`, `alash_viewed`. Cross-component sync via custom DOM events (`cart-updated`, `favorites-updated`).
...
### Hosting
- **EC2 `alash-electronics`** (13.51.198.130): PM2 process **`alash-electronics`** (id=0) on port **5000** (frontend). Project at `/home/ubuntu/alashed-shop/frontend/`.
...
```
DATABASE_URL                  # PostgreSQL RDS (eu-north-1)
NEXTAUTH_SECRET               # JWT signing (random 32-char string)
NEXTAUTH_URL                  # https://alash-electronics.kz (prod) / http://localhost:3000 (dev)
