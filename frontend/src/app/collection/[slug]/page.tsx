...
export async function generateMetadata({ params, searchParams }: Props): Promise<Metadata> {
  if (params.slug === 'all') {
    ...
    if (searchParams.search) {
      ...
      return {
        title: `Поиск: ${q}`,
        description: `Результаты поиска по запросу «${q}» в интернет-магазине Alash Electronics.`,
    ...
    return {
      ...
      description: 'Каталог всех электронных компонентов Alash Electronics. Arduino, Raspberry Pi, датчики, модули и многое другое.',
  ...
  const description = category.description
    ...
    : `${category.name} — купить в интернет-магазине Alash Electronics. Доставка по Казахстану.`
  const page = parseInt(searchParams.page || '1')
  ...
  return {
    ...
    openGraph: {
      title: `${baseTitle} — Alash Electronics`,
      description,
      url: canonicalUrl,
      locale: 'ru_KZ',
      images: [{ url: '/images/logo.png', width: 600, height: 300, alt: `${category.name} — Alash Electronics` }],
    },
    twitter: {
...
export default async function CollectionPage({ params, searchParams }: Props) {
  ...
  const CATEGORY_FAQ: Record<string, { q: string; a: string }[]> = {
    'gotovye-nabory-dlya-robototehniki': [
      ...
      { q: 'Какие наборы есть для школ и кружков робототехники?', a: 'В каталоге Alash Electronics есть LEGO Education SPIKE Prime, Microbit, mBot, наборы Keyestudio и STEM-комплекты для школьников 5–11 классов. Оптовые поставки для школ — через форму обратной связи.' },
      { q: 'Как быстро доставят набор по Казахстану?', a: 'По Алматы — Яндекс Курьер за 1–2 дня. По всему Казахстану — inDrive за 2–5 дней. Самовывоз: ул. Кыз Жибек, 104/1, пн–сб 12:00–20:00.' },
    ],
    'gotovye-nabory-arduino': [
      ...
      { q: 'Есть ли набор Arduino с инструкцией на русском?', a: 'Да. Расширенный обучающий набор Arduino от Alash Electronics идёт с инструкцией на русском языке на 200+ страниц и готовыми проектами.' },
      { q: 'Можно ли купить набор Arduino оптом для школы?', a: 'Да. Alash Electronics работает с образовательными организациями, кружками и STEM-центрами. Пишите на странице Контакты или Обратная связь для согласования условий.' },
    ],
    'arduino': [
      { q: 'Какую плату Arduino купить для первого проекта?', a: 'Для первого проекта рекомендуется Arduino UNO R3 — самая распространённая плата с большим сообществом и совместимостью с большинством шилдов и библиотек. Доступна в Alash Electronics.' },
      ...
      { q: 'Совместима ли Arduino с IDE на Mac и Linux?', a: 'Да. Arduino IDE доступна для Windows, macOS и Linux. Бесплатно скачивается с официального сайта arduino.cc. Все платы из каталога Alash Electronics поддерживают стандартную среду разработки.' },
      { q: 'Можно ли купить Arduino оптом в Казахстане?', a: 'Да, Alash Electronics принимает оптовые заявки для школ, кружков и компаний. Заполните форму обратной связи на странице Контакты — согласуем условия и счёт.' },
    ],
    'esp32': [
      ...
      { q: 'Какое питание нужно для ESP32?', a: 'ESP32 питается от 3.3 В (GPIO) или 5 В через USB. При Wi-Fi-передаче потребление до 250 мА. Рекомендуется использовать LDO-регулятор и конденсатор 100 мкФ для стабилизации. Модули в каталоге Alash Electronics поставляются с USB-интерфейсом.' },
      { q: 'Есть ли ESP32 с дисплеем или камерой?', a: 'Да. В каталоге Alash Electronics есть ESP32-CAM с OV2640-камерой и ESP32 с TFT-дисплеем. Также доступны модули ESP32-S3 с расширенной поддержкой USB и AI-ускорением.' },
    ],
    'raspberry-pi': [
      ...
      { q: 'Какой Raspberry Pi купить в 2024 году?', a: 'Raspberry Pi 4 Model B (2–8 ГБ ОЗУ) — лучший выбор для десктопа и сервера. Raspberry Pi Zero 2 W — компактный и дешёвый для IoT-проектов. Raspberry Pi 5 — новейшая модель с 2–8 ГБ ОЗУ и PCIe. Все модели доступны в Alash Electronics.' },
      ...
      { q: 'Нужно ли покупать аксессуары отдельно?', a: 'Плата Raspberry Pi поставляется без блока питания, microSD-карты и корпуса. В Alash Electronics доступны комплектующие: блок питания USB-C 5В/3А, SD-карты с предустановленной ОС, корпуса и охлаждение.' },
    ],
    'datchiki': [
      { q: 'Какие датчики чаще всего используют с Arduino?', a: 'Топ датчиков для Arduino: DHT11/DHT22 (температура и влажность), HC-SR04 (расстояние), PIR (движение), MQ-2/MQ-135 (газы), BMP280 (давление). Все модели есть в каталоге Alash Electronics.' },
      ...
      <main className="flex-1 min-w-0">
        ...
        {!isNonCanonicalUrl && (
          <div className="mt-8 pt-6 border-t text-sm text-gray-500 space-y-3">
            ...
            <p>
              Интернет-магазин Alash Electronics предлагает {total > 0 ? `${total} товар${total === 1 ? '' : total < 5 ? 'а' : 'ов'} в категории «${category.name}»` : `товары категории «${category.name}»`} с быстрой обработкой заказов.
              Доставка по Алматы — через Яндекс Курьер, по всему Казахстану — через inDrive.
              Самовывоз из магазина по адресу: ул. Кыз Жибек, 104/1, Алматы (пн–сб, 12:00–20:00).
              Оплата при получении картой или наличными, а также онлайн-перевод.
            </p>
            <p>
              Arduino, ESP32, датчики, модули, аккумуляторы и комплектующие — в одном месте. Работаем с 2019 года, доставляем по Алматы и всему Казахстану. Бесплатная доставка при заказе от 150 000 ₸.
            </p>
            {category.children && category.children.length > 0 && (
              <p>
            ...
            <p><strong>Как оформить заказ?</strong> Добавьте товар в корзину и укажите контакты. Оплата при получении наличными или картой. Доставка по Алматы — Яндекс Курьер, по Казахстану — inDrive.</p>
            <p><strong>Когда бесплатная доставка?</strong> При заказе от 150 000 ₸. Самовывоз: ул. Кыз Жибек, 104/1, пн–сб 12:00–20:00. Тел. +7 (700) 900-17-90.</p>
          </div>
        )}
    ...
    return (
      <div className="py-12 text-center space-y-3">
        ...
        <p className="text-sm text-gray-400 max-w-md mx-auto">
          Попробуйте изменить фильтры или посмотрите весь каталог. Alash Electronics доставляет Arduino, датчики и модули по Алматы и Казахстану.
        </p>
      </div>
    )
  }
