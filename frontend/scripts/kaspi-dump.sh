...
PIDFILE="/tmp/kaspi-dump.pid"
LOGFILE="/tmp/kaspi-dump.log"
...
MERCHANT_UID="${MERCHANT_UID:-30383258}"
...
SITE="${SITE:-https://alash-electronics.kz}"
...
case "${1:-status}" in
  ...
  install)
    ...
    PLIST_DST="$HOME/Library/LaunchAgents/kz.alash.kaspi-dump.plist"
    ...
    echo "✅ launchd-агент установлен и запущен (kz.alash.kaspi-dump)."
