...
// Источники: /tmp/croon-kaspi-dump.log (живой лог воркера) + прод-API статуса (позиции из БД).
// Env: PORT, LOG, SITE (default https://croon.kz), DUMPING_SECRET|CRON_SECRET.
...
const LOG = process.env.LOG || '/tmp/croon-kaspi-dump.log'
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'POST' && req.url === '/act/login') {
    ...
    exec('nohup ./scripts/kaspi-dump.sh login > /tmp/croon-kaspi-login.log 2>&1 &', { cwd: FRONTEND_DIR })
  ...
  if (req.method === 'POST' && req.url === '/act/creds') {
    ...
    req.on('end', () => {
      try {
        ...
        exec('nohup ./scripts/kaspi-dump.sh install > /tmp/croon-kaspi-login.log 2>&1 &', { cwd: FRONTEND_DIR })
